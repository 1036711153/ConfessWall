...
public class HotFragment extends Fragment implements IHotFragmentView {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                hotFragmentPresenter.FirstLoadingData(mhandler, getActivity());
                currentpage = 1;
            }
        }).start();
        //刷新加载数据
        mRefresh.setMaterialRefreshListener(new MaterialRefreshListener() {
            ...
            public void onRefreshLoadMore(MaterialRefreshLayout materialRefreshLayout) {
                //上拉刷新...
                hotFragmentPresenter.PullDownRefreshqueryData(mhandler, currentpage, getActivity());
                // 结束上拉刷新...
            }
        });
        adapter.setMyOnItemClickListener(new NewsAndHotAdapter.OnMyItemClickListener() {
            ...
            public void onSupportClick(View view, int position, MessageWall messageWall, boolean isclick, ImageView imageView, TextView textView) {
                if (isclick) {
                        ...
                        hotFragmentPresenter.ChangeCount(messageWall, getActivity(), messageWall.getCollection_count(), messageWall.getComment_count(), count);
                    } else {
            ...
            public void onCollectionClick(View view, int position, MessageWall messageWall, boolean isclick, ImageView imageView, TextView textView) {
                if (isclick) {
                    hotFragmentPresenter.CollectionOp(getActivity(), messageWall);
                    imageView.setImageResource(R.drawable.btn_star_on_pressed_holo_dark);
                    int count = Integer.parseInt(textView.getText().toString());
                    count++;
                    textView.setText("" + count);
                    hotFragmentPresenter.ChangeCount(messageWall, getActivity(), count, messageWall.getComment_count(), messageWall.getSupport_count());
                } else {
                    hotFragmentPresenter.DelCollection(getActivity(), messageWall);
